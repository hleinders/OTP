...
func TestTOTP(t *testing.T) {
	...
	otps := []OneTimePassword{
		{digit, step, time.Unix(0, 0), sha1.New},
		{digit, step, time.Unix(0, 0), sha256.New},
		{digit, step, time.Unix(0, 0), sha512.New},
	}
	...
	for i, exp := range expected {
		...
		if v := otp.HOTP(secret, otp.steps(now)); v != exp {
			t.Errorf("%s", fmt.Sprintf("%d", uint64(now.Unix()-otp.BaseTime.Unix())))
